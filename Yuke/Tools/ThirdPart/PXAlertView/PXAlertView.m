 ...
 buttonsShouldStack:(BOOL)shouldstack
    ...
    if (self) {
        ...
        self.titleLabel = [[UILabel alloc]
            initWithFrame:CGRectMake(AlertViewContentMargin, AlertViewVerticalElementSpace,
                                     AlertViewWidth - AlertViewContentMargin * 2, 44)];
        ...
        CGFloat messageLabelY = self.titleLabel.frame.origin.y + self.titleLabel.frame.size.height +
                                AlertViewVerticalElementSpace;

        // Optional Content View
        if (contentView) {
            self.contentView = contentView;
            self.contentView.frame = CGRectMake(0, messageLabelY, self.contentView.frame.size.width,
        ...
        // Message
        self.messageScrollView = [[UIScrollView alloc]
            initWithFrame:(CGRect){AlertViewContentMargin, messageLabelY,
                                   AlertViewWidth - AlertViewContentMargin * 2, 44}];
        self.messageScrollView.scrollEnabled = YES;

        self.messageLabel =
            [[UILabel alloc] initWithFrame:(CGRect){0, 0, self.messageScrollView.frame.size}];
        ...
        if (self.buttonsShouldStack) {
            ...
            if (otherTitles && [otherTitles count] > 0) {
                totalBottomHeight +=
                    (AlertViewButtonHeight + AlertViewLineLayerWidth) * [otherTitles count];
            }
        } else {
            totalBottomHeight += AlertViewButtonHeight;
        }
        ...
        self.messageScrollView.frame = (CGRect){
            self.messageScrollView.frame.origin, self.messageScrollView.frame.size.width,
            MIN(self.messageLabel.frame.size.height,
                self.alertWindow.frame.size.height - self.messageScrollView.frame.origin.y -
                    totalBottomHeight - AlertViewVerticalEdgeMinMargin * 2)};

        // Line
        CALayer* lineLayer = [self lineLayer];
        lineLayer.frame = CGRectMake(0, self.messageScrollView.frame.origin.y +
    ...
    if (self.isVisible) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8 &&
            ...
            keyboardFrameBeginRect =
                (CGRect){keyboardFrameBeginRect.origin.y, keyboardFrameBeginRect.origin.x,
                         keyboardFrameBeginRect.size.height, keyboardFrameBeginRect.size.width};
        }
        CGRect interfaceFrame = [self frameForOrientation];

        if (interfaceFrame.size.height - keyboardFrameBeginRect.size.height <=
            _alertView.frame.size.height + _alertView.frame.origin.y) {
            [UIView animateWithDuration:.35
                                  delay:0
                                options:0x70000
                             animations:^(void) {
                                 _alertView.frame =
                                     (CGRect){_alertView.frame.origin.x,
                                              interfaceFrame.size.height -
                                                  keyboardFrameBeginRect.size.height -
                                                  _alertView.frame.size.height - 20,
                                              _alertView.frame.size};
                             }
                             completion:nil];
        }
    }
...
- (CGRect)frameForOrientation {
    UIWindow* window = [[UIApplication sharedApplication].windows count] > 0
...
- (UIButton*)genericButton {
    ...
    [button addTarget:self
                  action:@selector(clearBackgroundColorForButton:)
...
- (void)resizeViews {
    ...
    self.alertView.frame = CGRectMake(self.alertView.frame.origin.x, self.alertView.frame.origin.y,
...
- (void)setBackgroundColorForButton:(id)sender {
    [sender setBackgroundColor:
                [UIColor colorWithRed:94 / 255.0 green:196 / 255.0 blue:221 / 255.0 alpha:1.0]];
}
...
//                }
                //当点击背景view时，只需要消失就可以了，不需要调用completion
                if (sender && sender != self.tap) {
                    self.completion(cancelled, buttonIndex);
                }
            }
...
- (NSInteger)addButtonWithTitle:(NSString*)title {
    ...
    if (!self.cancelButton) {
        ...
        self.cancelButton.frame =
            CGRectMake(0, self.buttonsY, AlertViewWidth, AlertViewButtonHeight);
        ...
        self.cancelButton.frame =
            CGRectMake(0, lastButtonYOffset, AlertViewWidth, AlertViewButtonHeight);
        ...
        if ([self.buttons count] == 2) {
            ...
            CALayer* lineLayer = [self lineLayer];
            lineLayer.frame = CGRectMake(0, self.buttonsY + AlertViewButtonHeight, AlertViewWidth,
                                         AlertViewLineLayerWidth);
            [self.alertView.layer addSublayer:lineLayer];
            lastButton.frame = CGRectMake(0, self.buttonsY + AlertViewButtonHeight, AlertViewWidth,
                                          AlertViewButtonHeight);
            self.cancelButton.frame =
                CGRectMake(0, self.buttonsY, AlertViewWidth, AlertViewButtonHeight);
        }
        ...
        self.verticalLine.frame = CGRectMake(AlertViewWidth / 2, self.buttonsY,
                                             AlertViewLineLayerWidth, AlertViewButtonHeight);
        [self.alertView.layer addSublayer:self.verticalLine];
        button.frame = CGRectMake(AlertViewWidth / 2, self.buttonsY, AlertViewWidth / 2,
                                  AlertViewButtonHeight);
        self.otherButton = button;
        self.cancelButton.frame =
            CGRectMake(0, self.buttonsY, AlertViewWidth / 2, AlertViewButtonHeight);
