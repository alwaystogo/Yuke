...
//确定按钮、取消按钮、PickerView的父视图
- (UIView *)backView {
    if (!_backView) {
        _backView = [[UIView alloc]init];
        _backView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, kPickerViewHeight);
...
- (UIPickerView *)pickerView {
    if (!_pickerView) {
        ...
        _pickerView.frame           = CGRectMake(0, 50, ScreenWidth, kPickerViewHeight - 50);
...
- (void)determineBtnAction:(UIButton *)button {
    ...
    if (self.determineBtnBlock) {
        ...
        if (_shengArray.count != 0 && _shiArr.count != 0 && _xianArr.count != 0) {
