     ...
     successCompress:(void(^)(NSURL *))successCompress  //saveState 是否保存视频到相册
{
    ...
    NSString *oldPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, true) lastObject] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",self.videoName]];
    NSFileManager *manager = [NSFileManager defaultManager];
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        if ([exportSession status] == AVAssetExportSessionStatusCompleted) {
            ...
            //压缩成功视频流回调回去
            successCompress([NSData dataWithContentsOfURL:[self compressedURL]].length > 0?[self compressedURL]:nil);
        }else{
            //压缩失败的回调
            successCompress(nil);
        }
